...
Provides convenient access to global functions:
...
var scriptWithBase = @"
    public class MyMapping : ScriptBase, IMapping
    {
        public Task<ScriptResponse> InputHandler(WorkflowTask task, ScriptContext context)
        {
            var apiKey = GetSecret(""dapr_store"",""secret_store"", ""api_key""); // Clean syntax!
            
            return Task.FromResult(new ScriptResponse
            {
                Data = new { apiKey = apiKey },
                Headers = null
            });
        }
