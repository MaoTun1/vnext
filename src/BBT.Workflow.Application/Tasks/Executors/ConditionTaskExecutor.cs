    ...
    ILogger<ConditionTaskExecutor> logger) : TaskExecutor(scriptEngine, logger), ITaskExecutor
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Logger.LogInformation("Starting condition task execution for task {TaskKey}", task.Key);
        
        try
        {
            Logger.LogDebug("Compiling condition script code for task {TaskKey}", task.Key);
            ...
            stopwatch.Stop();
            Logger.LogInformation("Condition task {TaskKey} completed successfully in {Duration}ms with result: {Result}", 
        ...
        {
            stopwatch.Stop();
            Logger.LogError(ex, "Error occurred during condition task {TaskKey} execution after {Duration}ms", 
