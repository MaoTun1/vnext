...
/// <summary>
/// Factory for creating HTTP tasks used in trigger transition tasks.
...
public interface ITriggerTransitionHttpTaskFactory
{
    ...
    /// </summary>
    /// <param name="task">The trigger transition task containing configuration (StartTask, DirectTriggerTask, or GetInstanceDataTask).</param>
    ...
    /// <param name="method">The HTTP method (POST, PATCH, GET, etc.).</param>
    ...
    /// </summary>
    /// <param name="task">The trigger transition task (DirectTriggerTask or GetInstanceDataTask).</param>
    ...
    /// 2. If TriggerKey is provided, queries the instance by key to get the ID (except for GetInstanceDataTask which uses key directly)
