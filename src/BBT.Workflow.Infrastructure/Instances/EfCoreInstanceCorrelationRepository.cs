        ...
        IInstanceCorrelationRepository
{
    ...
    /// </returns>
    public async Task<List<InstanceCorrelation>> FindActiveByParentInstanceIdAsync(
    ...
    /// </returns>
    public async Task<InstanceCorrelation?> FindBySubFlowInstanceIdAsync(
        Guid subFlowInstanceId, 
        CancellationToken cancellationToken = default)
    {
        return await (await GetDbSetAsync())
            .FirstOrDefaultAsync(c => c.SubFlowInstanceId == subFlowInstanceId, cancellationToken);
    }
    ...
    /// </returns>
    public async Task<List<InstanceCorrelation>> FindIncompleteByParentAsync(
        Guid parentInstanceId, 
        string parentState, 
        CancellationToken cancellationToken = default)
    {
    ...
    /// </returns>
    public async Task<bool> HasIncompleteCorrelationsAsync(
        Guid parentInstanceId, 
        CancellationToken cancellationToken = default)
    {
        return await (await GetDbSetAsync())
            .AnyAsync(c => c.ParentInstanceId == parentInstanceId && !c.IsCompleted, cancellationToken);
    }
    ...
    /// </returns>
    public async Task<bool> HasActiveBlockingSubFlowsAsync(
        Guid parentInstanceId,
        CancellationToken cancellationToken = default)
    {
        return await (await GetDbSetAsync())
            .AnyAsync(c => c.ParentInstanceId == parentInstanceId 
                          && !c.IsCompleted 
                          && c.SubFlowType == SubFlowType.SubFlow, cancellationToken);
    }
    ...
    /// </returns>
    public async Task<InstanceCorrelation?> FindActiveBlockingSubFlowAsync(
        Guid parentInstanceId,
        string parentState,
        CancellationToken cancellationToken = default)
    {
        return await (await GetDbSetAsync())
            .FirstOrDefaultAsync(c => c.ParentInstanceId == parentInstanceId
                                     && c.ParentState == parentState
                                     && !c.IsCompleted
                                     && c.SubFlowType == SubFlowType.SubFlow, cancellationToken);
    }
