...
/// <summary>
/// ClickHouse data sink implementation for Instance entities
/// </summary>
public class ClickHouseInstanceDataSink : AbstractDataSink<Instance>, IDataSink
{
    private readonly ClickHouseConfiguration _configuration;
    private readonly HttpClient _httpClient;
    private readonly ConcurrentQueue<ClickHouseInstanceData> _queue;
    private readonly SemaphoreSlim _flushSemaphore;
    private readonly Timer _flushTimer;

    /// <summary>
    /// Initializes a new instance of the ClickHouseInstanceDataSink class
    /// </summary>
    ...
    public ClickHouseInstanceDataSink(
        IOptions<ClickHouseConfiguration> configuration,
        HttpClient httpClient,
        ILogger<ClickHouseInstanceDataSink> logger)
        : base(logger)
    {
        _configuration = configuration?.Value ?? throw new ArgumentNullException(nameof(configuration));
        _httpClient = httpClient ?? throw new ArgumentNullException(nameof(httpClient));
        _queue = new ConcurrentQueue<ClickHouseInstanceData>();
    ...
    /// </summary>
    public override string Name => "ClickHouse-Instance";
    ...
    protected override async Task OnFlushAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            var dataToFlush = new List<ClickHouseInstanceData>();
    ...
    private async Task FlushTimerCallback()
    {
        ...
        {
            Logger.LogError(ex, "Error during scheduled flush in ClickHouse instance data sink");
        }
    }
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    private async Task SendDataToClickHouseAsync(string tableName, List<ClickHouseInstanceData> data, CancellationToken cancellationToken)
    {
        var retryCount = 0;
        while (retryCount < _configuration.RetryAttempts)
        {
            ...
            {
                Logger.LogError(ex, "Failed to send instance data to ClickHouse table {TableName} after {RetryCount} attempts", 
    ...
    /// <returns>ClickHouse instance data</returns>
    private static ClickHouseInstanceData MapInstanceToClickHouse(Instance instance, DataSinkOperation operation)
    {
        return new ClickHouseInstanceData
        {
            ...
            Tags = JsonSerializer.Serialize(instance.Tags ?? new List<string>()),
...
/// <summary>
/// Custom JSON converter for DateTime fields to ClickHouse format
/// </summary>
public class ClickHouseDateTimeConverter : JsonConverter<DateTime>
{
    ...
    {
        // ClickHouse expects DateTime64(3) format: YYYY-MM-DD HH:MM:SS.fff
        writer.WriteStringValue(value.ToString("yyyy-MM-dd HH:mm:ss.fff"));
    }
}

/// <summary>
/// Custom JSON converter for nullable DateTime fields to ClickHouse format
/// </summary>
...
/// <summary>
/// Custom JSON converter for Guid fields to ClickHouse format
/// </summary>
public class ClickHouseGuidConverter : JsonConverter<Guid>
{
    ...
    {
        // ClickHouse expects GUID in uppercase format
        writer.WriteStringValue(value.ToString().ToUpperInvariant());
    }
}

/// <summary>
/// Custom JSON converter for nullable Guid fields to ClickHouse format
/// </summary>
