...
/// <summary>
/// ClickHouse data sink implementation for InstanceTask entities
/// </summary>
public class ClickHouseInstanceTaskDataSink : AbstractDataSink<InstanceTask>, IDataSink
{
    private readonly ClickHouseConfiguration _configuration;
    private readonly HttpClient _httpClient;
    private readonly ConcurrentQueue<ClickHouseInstanceTaskData> _queue;
    private readonly SemaphoreSlim _flushSemaphore;
    private readonly Timer _flushTimer;

    /// <summary>
    /// Initializes a new instance of the ClickHouseInstanceTaskDataSink class
    /// </summary>
    ...
    public ClickHouseInstanceTaskDataSink(
        IOptions<ClickHouseConfiguration> configuration,
        HttpClient httpClient,
        ILogger<ClickHouseInstanceTaskDataSink> logger)
        : base(logger)
    {
        _configuration = configuration?.Value ?? throw new ArgumentNullException(nameof(configuration));
        _httpClient = httpClient ?? throw new ArgumentNullException(nameof(httpClient));
        _queue = new ConcurrentQueue<ClickHouseInstanceTaskData>();
    ...
    /// </summary>
    public override string Name => "ClickHouse-InstanceTask";
    ...
    protected override async Task OnInsertAsync(InstanceTask entity, CancellationToken cancellationToken)
    {
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Task representing the async operation</returns>
    protected override async Task OnUpdateAsync(InstanceTask entity, CancellationToken cancellationToken)
    {
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Task representing the async operation</returns>
    protected override async Task OnDeleteAsync(InstanceTask entity, CancellationToken cancellationToken)
    {
    ...
    protected override async Task OnFlushAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            var dataToFlush = new List<ClickHouseInstanceTaskData>();
    ...
    private async Task FlushTimerCallback()
    {
        ...
        {
            Logger.LogError(ex, "Error during scheduled flush in ClickHouse instance task data sink");
        }
    }
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    private async Task SendDataToClickHouseAsync(string tableName, List<ClickHouseInstanceTaskData> data, CancellationToken cancellationToken)
    {
        var retryCount = 0;
        while (retryCount < _configuration.RetryAttempts)
        {
            ...
            {
                Logger.LogError(ex, "Failed to send instance task data to ClickHouse table {TableName} after {RetryCount} attempts", 
    ...
    /// <returns>ClickHouse instance task data</returns>
    private static ClickHouseInstanceTaskData MapInstanceTaskToClickHouse(InstanceTask task, DataSinkOperation operation)
    {
        return new ClickHouseInstanceTaskData
        {
            ...
            Request = JsonSerializer.Serialize(task.Request),
            Response = JsonSerializer.Serialize(task.Response),
