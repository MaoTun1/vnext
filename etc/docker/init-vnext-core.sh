...
echo "[vnext-init] npm registry: ${REGISTRY}"

# If using private registry, NPM_TOKEN is expected
# Example: REGISTRY should be in format https://registry.npmjs.org/ (with trailing /)
if [[ -n "${NPM_TOKEN:-}" ]]; then
  echo "//$(echo "${REGISTRY}" | sed -E 's#^https?://##' ):_authToken=${NPM_TOKEN}" > ~/.npmrc
...
if [[ ! -f package.json ]]; then
  echo "[vnext-init] No package.json found. Creating..."
  npm init -y >/dev/null 2>&1
fi

# Installation
echo "[vnext-init] Installing @burgan-tech/vnext-core-runtime@${VERSION} ..."
npm install "@burgan-tech/vnext-core-runtime@${VERSION}" \
  --no-audit --no-fund --loglevel=warn --save-exact --no-save

# Show installed version
node -e "console.log('[vnext-init] Installed version:', require('@burgan-tech/vnext-core-runtime/package.json').version)"
...
# Get APP_DOMAIN from environment variable, default to "core" if not set
